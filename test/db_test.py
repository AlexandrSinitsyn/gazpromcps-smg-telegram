...
class JobServiceTestCase(unittest.TestCase):
    ...
    def test_presence(self):
        test_job = Job(1, 'test stage', 'test gen_plan', 'test master', 'test title', 'test measurement', True, datetime.now())
        self.excel_service.import_data([test_job])

        found = self.job_service.get_by_params('test stage', 'test gen_plan', 'test master', 'test title')
        self.assertEqual(found.title, 'test title')

        self.excel_service.delete_all()

    def test_found_list(self):
        test_jobs = [Job(i, str(i), str(i), str(i), str(i), str(i), True, datetime.now()) for i in range(1, 10)]
...
class ExcelServiceTestCase(unittest.TestCase):
    ...
    def test_export(self):
        ...
        for i in range(10):
            ...
            job = self.job_service.get_by_params(idx.stage, idx.gen_plan, idx.master, idx.title)
